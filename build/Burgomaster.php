...
EOT
     ...
     * @throws \RuntimeException if the file cannot be written
     */
    function createAutoloaderDev($files = array(), $filename = 'autoloader.php', $devDest) {
        $sourceDir = realpath($this->stageDir);
        $iter = new \RecursiveDirectoryIterator($sourceDir);
        $iter = new \RecursiveIteratorIterator($iter);
        ...
        $destFile = $devDest . '/' . $filename;
        $this->debug("Writing DEV autoloader to {$devDest}");
        ...
        fwrite($h, "\$mapping = array(\n");
        foreach ($classMap as $c =>  $f) {
            $filePath = '__DIR__ . \'/src' . substr($f, strlen("__DIR__ . '/CloudRail"), strlen($f));
            fwrite($h, "    '$c' => $filePath,\n");
        }
        fwrite($h, ");\n\n");
        fwrite($h, <<<EOT
